...
<% 
	...
	ps1.setSchool("대졸");
	ps1.setColor("red");
	ps1.setFood("초콜릿,마이쮸,초코파이".split("\\,")); // ","를 구분자로 문자열을 잘라서 배열로 리턴해줌.
	
	PersonDTO ps2 = new PersonDTO();
	ps2.setName("이미자");
	ps2.setSchool("초대졸");
	ps2.setColor("blue");
	ps2.setFood("육회비빔밥,광어회,조개구이,참이슬".split("\\,")); // ","를 구분자로 문자열을 잘라서 배열로 리턴해줌.
	
	PersonDTO ps3 = new PersonDTO();
	ps3.setName("강산애");
	ps3.setSchool("고졸");
	ps3.setColor("yellow");
	ps3.setFood("라면,떡볶이,순대,피자".split("\\,")); // ","를 구분자로 문자열을 잘라서 배열로 리턴해줌.
	...
	RequestDispatcher dispatcher = request.getRequestDispatcher("05View_result.jsp"); // 05View_result.jsp파일만 nameArr1 값을 꺼내어 볼 수 있음. 
