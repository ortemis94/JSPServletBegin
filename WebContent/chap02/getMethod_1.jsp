...
<form name="memberFrm" action="<%= request.getContextPath()%>/getMethod_1.do" method="get"> 
  또는
--%>
<form name="memberFrm" action="<%= ctxPath%>/getMethod_1.do" method="get">
<%-- 
     submit 을 하면 <%= ctxPath%>/getMethod_1.do 로 보내어서 처리를 요청한다.
     /JSPServletBegin/getMethod_1.do 은 누가 처리를 해주는지 배치서술자인 web.xml 에 기술되어져 있어야 한다.
	 web.xml 에 가보면 <servlet-mapping>에 URL 패턴으로 /getMethod_1.do 가 기술되어져 있고,
